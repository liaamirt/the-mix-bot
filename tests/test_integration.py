...
async def test_create_new_booking(db_connection):
    ...
    GIVEN: Ізольоване середовище бази даних та валідний набір даних для нового клієнта.
    WHEN:  Система виконує асинхронний SQL-запит INSERT для фіксації бронювання в БД.
    THEN:  Запис успішно з'являється в базі, а збережена дата візиту залишається консистентною.
    """
